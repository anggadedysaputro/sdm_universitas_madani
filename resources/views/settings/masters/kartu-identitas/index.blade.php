...
@endsection
@section('breadcrumb')
    <x-bread-crumbs breadcrumbtitle="settings.masters.kartu-identitas.index"/>
@endsection
@section('page-title')
    Kartu identitas
@endsection
...
<a id="tambah" class="btn btn-primary" role="button">
	<i class="ti ti-plus"></i> Tambah kartu identitas
</a>
@endsection
@section('search')
...
<div class="card p-3">
    <div class="card-table table-responsive">
        <table class="table table-vcenter" id="table-main">
            <thead>
            <tr>
                <th class="col-md-1 text-center">Kode kartu identitas</th>
...
<div class="offcanvas offcanvas-end" tabindex="-1" aria-labelledby="offcanvasEndLabel" id="canvas-main">
    <div class="offcanvas-header">
		<h2 class="offcanvas-title" id="offcanvasEndLabel">Input kartu identitas</h2>
    ...
    <div class="offcanvas-body">
        <form action="#" id="form-main">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="mb-3">
                        <label class="form-label">Nama kartu identitas</label>
                        <input type="text" class="form-control" name="keterangan" placeholder="Input nama kartu identitas" fdprocessedid="tigmx5" required>
                        <div class="invalid-feedback">Nama kartu identitas belum diisi</div>
                    </div>
                </div>
            </div>
        </form>
    ...
    <script type="module">
        ...
        class Helper {
            ...
            simpan(){
                ...
                let url = (mode == 'edit' ? '{{ route('settings.masters.kartu-identitas.edit') }}' : '{{ route('settings.masters.kartu-identitas.store') }}');
            ...
            static hapus(e){
                ...
                Swal.fire({
                    ...
                    if(result.isConfirmed){
                        $.ajax({
                            url : "{{ route('settings.masters.kartu-identitas.delete') }}",
                            method : "DELETE",
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
        ...
        export default class Index extends Helper{
            ...
            constructor() {
                ...
                Index.DT_Main=$("#table-main").DataTable({
                    ajax : {
                        url : "{{ route('settings.masters.kartu-identitas.data') }}",
                        method : "POST",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
